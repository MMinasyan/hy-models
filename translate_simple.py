...
from preproc import armchars, clean_bytes, armchars_l
import re
import random
import difflib
import pickle
...
hy_tokenizer = AutoTokenizer.from_pretrained("facebook/nllb-200-distilled-600M", src_lang="hye_Armn", max_len=256)
en_tokenizer = AutoTokenizer.from_pretrained("facebook/nllb-200-distilled-600M", max_len=256)

def build_nllb(device):
    ...
    if model2 != None:
        ...
        for key, re_translated_text in zip(re_keys_for_translation, re_translated_texts):
            if verify_bt(groups[key]['string'], re_translated_text):
                groups[key]['bt'] = randomize_chars(re_translated_text)
            else:
                groups[key]['bt'] = groups[key]['string']
                skipped += 1
